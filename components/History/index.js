...
class History extends Component {
  ...
  deleteHistoryItem = id => {
    ...
    const results = searchResults.filter(eachItemId => eachItemId.id !== id)
  ...
  render() {
    ...
    if (filteredResults.length !== 0) {
      renderedFunction = (
        <ul className="history-list-container">
          {filteredResults.map(eachList => (
            <HistoryItem
              historyItemList={eachList}
    ...
    return (
      <div className="main-container">
        ...
        <div className="details-container">{renderedFunction}</div>
      </div>
    )
  }
}
